 ...
 * methods. These methods will be accessible on the types the
 ...
 * ```
 *
 * Would be called like `$type->doSomething($arg1, $arg2);`.
 *
 * ### Callback methods
 *
 * Behaviors can listen to any events fired on a Type. By default
 ...
 *   Fired when the rules checking object for the type is being built. You can use this
 ...
 * event fired from your type classes including custom application
 * specific ones.
 *
 ...
 * ### Finder methods
 *
 * Behaviors can provide finder methods that hook into a type's
 ...
 * ```
 *
 * @see \Cake\ElasticSearch\Type::addBehavior()
...
class Behavior implements EventListenerInterface
{
     ...
     *
     * @param \Cake\ElasticSearch\Type $type The type this behavior is attached to.
     * @param array $config The config for this behavior.
     */
    public function __construct(Type $type, array $config = [])
    {
    ...
    /**
     * Get the type instance this behavior is bound to.
     *
     * @return \Cake\ElasticSearch\Type The bound type instance.
     */
     ...
     * ```
     *
     * With the above example, a call to `$type->find('this')` will call `$Behavior->findThis()`
     * and a call to `$type->find('alias')` will call `$Behavior->findMethodName()`
     *
     ...
     * ```
     *
     * With the above example, a call to `$type->method()` will call `$Behavior->method()`
     * and a call to `$type->aliasedmethod()` will call `$Behavior->somethingElse()`
     *
     * It is recommended, though not required, to define implementedFinders in the config property
     * of child classes such that it is not necessary to use reflections to derive the available
     * method list. See core behaviors for examples
